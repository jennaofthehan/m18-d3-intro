...
$(function() {
  ...
  paragraphs.enter().append('p').text(function(p){return p.name}).style('color', function(p){return p.favColor});
  ...
  setTimeout(function(){
    ...
    // Make sure to specify the `name` attribute as an identifying characteristic in your data-join.
    paragraphs = myDiv.selectAll('p').data(people2, function(p){return p.name});

    // Determine which people are no longer represented in the data (`exit`), and transition their opacity to 0 and remove them.
    paragraphs.exit().transition().style('opacity', '0').remove();
